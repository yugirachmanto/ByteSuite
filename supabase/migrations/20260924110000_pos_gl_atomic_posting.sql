-- Fix: POS sale rejected with "Unbalanced GL entries" once a tax rate is set.
-- GL revenue/payment/tax lines were inserted independently; when an account
-- could not be resolved the remaining lines were still posted, leaving the
-- journal unbalanced. Now posted all-or-nothing (else gl_status=pending_mapping).
...
BEGIN
  ...
  IF v_revenue_coa_id IS NULL OR (p_tax_amount > 0 AND v_tax_coa_id IS NULL) THEN
    v_gl_complete := FALSE;
  END IF;
...
BEGIN
  ...
  LOOP
    ...
    IF v_revenue_coa_id IS NULL OR (ord.tax_amount > 0 AND v_tax_coa_id IS NULL) THEN
      v_ok := FALSE;
    END IF;
