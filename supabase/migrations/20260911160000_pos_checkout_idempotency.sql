-- POS Fase 10: offline resilience — idempotency support so a retried
-- checkout (queued locally during a network blip, or replayed after page
-- reload) never double-posts an order, double-deducts stock, or
-- double-posts GL entries.

ALTER TABLE pos_orders ADD COLUMN client_request_id UUID;

-- Defense-in-depth against a concurrent duplicate racing the RPC's own
-- idempotency check below.
CREATE UNIQUE INDEX pos_orders_cashier_client_request_id_key
  ON pos_orders(cashier_id, client_request_id)
  WHERE client_request_id IS NOT NULL;

-- ============================================================
-- process_pos_order — add p_client_request_id idempotency short-circuit.
-- Everything else in this function is unchanged from the Fase 6 version.
-- p_shift_id is stamped as-is with no status check, same as before — that
-- enforcement lives only in the API route, which is what changes to allow
-- a late-arriving queued sale to post into its original (now possibly
-- closed) shift.
-- ============================================================
CREATE OR REPLACE FUNCTION public.process_pos_order(p_org_id uuid, p_outlet_id uuid, p_cashier_id uuid, p_payment_method text, p_subtotal numeric, p_tax_amount numeric, p_total_amount numeric, p_lines jsonb, p_shift_id uuid DEFAULT NULL, p_client_request_id uuid DEFAULT NULL)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
  ...
  INSERT INTO pos_orders (org_id, outlet_id, cashier_id, status, subtotal, tax_amount, total_amount, payment_method, shift_id, client_request_id)
  VALUES (p_org_id, p_outlet_id, p_cashier_id, 'completed', p_subtotal, p_tax_amount, p_total_amount, p_payment_method, p_shift_id, p_client_request_id)
