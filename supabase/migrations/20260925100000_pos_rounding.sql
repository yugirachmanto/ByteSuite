-- POS total rounding (always rounded UP to a per-org step).
-- organizations.pos_rounding_step: 0 = off, else 100 / 500 / 1000.
-- pos_orders.rounding_amount: added on top of subtotal + tax; credited to the
-- 'pos_rounding' account mapping (falls back to the revenue account).
ALTER TABLE organizations ADD COLUMN IF NOT EXISTS pos_rounding_step INTEGER NOT NULL DEFAULT 0
  CHECK (pos_rounding_step IN (0, 100, 500, 1000));
ALTER TABLE pos_orders ADD COLUMN IF NOT EXISTS rounding_amount NUMERIC NOT NULL DEFAULT 0;

-- New trailing parameter changes the signature: drop the old one so the
-- call doesn't become ambiguous between two overloads.
DROP FUNCTION IF EXISTS public.process_pos_order(uuid, uuid, uuid, jsonb, numeric, numeric, numeric, jsonb, uuid, uuid, text, numeric, numeric, text);
...
BEGIN
  ...
  IF v_revenue_coa_id IS NULL OR (p_tax_amount > 0 AND v_tax_coa_id IS NULL) OR (COALESCE(p_rounding_amount, 0) > 0 AND v_rounding_coa_id IS NULL) THEN
    v_gl_complete := FALSE;
  END IF;
...
BEGIN
  ...
  LOOP
    ...
    IF v_revenue_coa_id IS NULL OR (ord.tax_amount > 0 AND v_tax_coa_id IS NULL) OR (ord.rounding_amount > 0 AND v_rounding_coa_id IS NULL) THEN
      v_ok := FALSE;
    END IF;

    IF v_ok THEN
      IF NOT EXISTS (SELECT 1 FROM gl_entries WHERE reference_id = ord.id AND coa_id = v_revenue_coa_id AND credit > 0) THEN
        FOR v_debit IN SELECT value FROM jsonb_array_elements(v_debits)
        LOOP
          INSERT INTO gl_entries (outlet_id, entry_date, coa_id, debit, credit, reference_id, reference_type, description)
