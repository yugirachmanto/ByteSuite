-- Fix regression introduced by the Fase 1 negative-stock guard: the
-- no-BOM direct-deduct branch of process_pos_order didn't check the sold
-- item's own is_inventory flag before checking/deducting stock. A
-- direct-expense finished item (is_inventory = false, e.g. a service
-- charge or a combo sold without ever being goods-received) has no
-- inventory_balance row by design — the new guard was hard-blocking every
-- sale of such an item with "Insufficient stock: ... needs N but only 0
-- available". Mirrors the same COALESCE(is_inventory, true) skip already
-- used for BOM inputs earlier in this same function.
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      -- Skip ingredients that aren't inventory-tracked (direct-expense inputs) —
      -- same COALESCE(is_inventory, true) default as post_production/process_pos_order elsewhere.
    ...
    -- unless the item itself is marked as not inventory-tracked
    -- (direct-expense, e.g. a service charge or combo never goods-received).
