...
export default function PurchaseOrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleRelease = async () => {
    ...
    setBusy(false)
    if (error) { toast.error(error.message); return }
    toast.success(`Purchase order released as ${poNumber}.`)
    fetchPo()
  }

  if (loading) return <div className="py-20 text-center text-zinc-500 text-sm">Loading purchase order…</div>
  if (!po) return (
    <div className="py-20 text-center space-y-4">
      <p className="text-zinc-400">Purchase order not found.</p>
      <Link href="/purchasing/po"><Button variant="outline" className="border-zinc-800 text-zinc-300">Back to Purchase Orders</Button></Link>
    </div>
  )
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-800 pb-5">
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <Badge variant="outline" className={STATUS_BADGE[po.status] || ''}>{po.status.replace('_', ' ')}</Badge>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {canReceive && (
            <Button onClick={() => router.push(`/purchasing/po/${poId}/receive`)} variant="outline" className="border-zinc-700 text-zinc-300 hover:bg-zinc-800 text-xs gap-1.5">
              <PackagePlus className="h-3.5 w-3.5" /> Receive Goods
            </Button>
          )}
          {canMatchInvoice && (
            <Button onClick={() => router.push(`/purchasing/po/${poId}/match-invoice`)} className="bg-zinc-100 text-zinc-900 hover:bg-zinc-200 text-xs gap-1.5">
              <FileText className="h-3.5 w-3.5" /> Match Invoice
            </Button>
          )}
          {matchedInvoice && (
            <Button onClick={() => router.push(`/invoices/${matchedInvoice.id}/review`)} variant="outline" className="border-zinc-700 text-zinc-300 hover:bg-zinc-800 text-xs gap-1.5">
              <FileText className="h-3.5 w-3.5" /> View Matched Invoice
            </Button>
          )}
        </div>
      </div>
      ...
      {receipts.length > 0 && (
        <div className="space-y-2">
          <h3 className="text-sm font-semibold text-zinc-200">Goods Receipts</h3>
              ...
              return (
                <div key={gr.id} className="flex items-center justify-between px-4 py-3 text-sm">
                  <button onClick={() => router.push(`/purchasing/gr/${gr.id}`)} className="text-zinc-200 hover:text-indigo-400 transition-colors">
                    {format(new Date(gr.receipt_date), 'dd MMM yyyy')} — {qty} units
                  </button>
