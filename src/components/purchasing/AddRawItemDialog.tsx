...
export function AddRawItemDialog({ open, onOpenChange, orgId, accounts, onCreated }: AddRawItemDialogProps) {
  ...
  const handleSave = async () => {
    ...
    if (!defaultCoaId) {
      toast.error(t('purchasing.addItemDialog.errAccountRequired'))
      return
    }
    ...
    try {
      ...
      toast.success(t('purchasing.addItemDialog.successCreated', { name: data.name }))
      onCreated(data)
      reset()
      onOpenChange(false)
    } catch (err: any) {
      toast.error(err.message || t('purchasing.addItemDialog.errFailedCreate'))
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-zinc-950 border-zinc-800 sm:max-w-lg">
        <DialogHeader>
          <DialogTitle className="text-zinc-100">{t('purchasing.addItemDialog.title')}</DialogTitle>
          <DialogDescription className="text-zinc-400">
            {t('purchasing.addItemDialog.description')}
          </DialogDescription>
        </DialogHeader>

        <div className="space-y-4 py-2">
          <div className="space-y-2">
            <label className="text-xs text-zinc-500 font-medium uppercase">{t('purchasing.addItemDialog.itemNameLabel')}</label>
            <Input
              autoFocus
              value={name}
              onChange={(e) => setName(e.target.value)}
              className="bg-zinc-900 border-zinc-800 h-9"
              placeholder={t('purchasing.addItemDialog.itemNamePlaceholder')}
            />
          </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5 rounded-lg border border-indigo-500/20 bg-indigo-500/[0.04] p-2.5">
                <label className="flex items-center gap-1.5 text-[11px] font-semibold text-indigo-400">
                  <ShoppingCart className="h-3.5 w-3.5" /> {t('purchasing.addItemDialog.purchaseUnitLabel')}
                </label>
                <Input
                  value={purchaseUnit}
                  ...
                  placeholder="e.g. KG"
                />
                <p className="text-[9px] text-zinc-500 leading-snug">{t('purchasing.addItemDialog.purchaseUnitHint')}</p>
              </div>
              ...
              <div className="space-y-1.5 rounded-lg border border-emerald-500/20 bg-emerald-500/[0.04] p-2.5">
                <label className="flex items-center gap-1.5 text-[11px] font-semibold text-emerald-400">
                  <PackageCheck className="h-3.5 w-3.5" /> {t('purchasing.addItemDialog.storageUnitLabel')}
                </label>
                <select
                  value={unit}
                  onChange={(e) => {
                ...
                <p className="text-[9px] text-zinc-500 leading-snug">{t('purchasing.addItemDialog.storageUnitHint')}</p>
              </div>
            </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSave} disabled={saving || !name.trim() || !defaultCoaId} className="bg-emerald-600 hover:bg-emerald-700 text-white">
            ...
            {t('purchasing.addItemDialog.saveButton')}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
