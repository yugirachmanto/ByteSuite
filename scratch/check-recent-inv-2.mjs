...
async function check() {
  ...
  // We fetch the latest invoice to see if they just posted it
  const { data: invs } = await supabase.from('invoices').select('id, invoice_no, total, status').order('created_at', { ascending: false }).limit(3)
